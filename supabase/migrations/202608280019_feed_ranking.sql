...
-- COMM-110 (ranked feed), COMM-111 (scopes), COMM-112 (diversity),
-- COMM-113 (cursor pagination) and COMM-115 (the club top area read).
--
-- One function, public.feed_page(), replaces the chronological
-- public.community_feed read cloud.js has used until now. Everything the
-- feed decides - what is visible, what it is worth, what order it lands in,
-- and where the next page starts - happens here, because a client that
-- re-sorts a ranked list is a client that silently disagrees with the
-- server about what the member already saw.
--
-- WHY A KEYSET ON SCORE AND NOT AN OFFSET
-- The page boundary is an opaque token carrying the session anchor, the
-- last row's score, its published_at and its id. Two properties follow:
--   * the anchor freezes now(), so the recency term - and therefore every
--     score - is identical for every page of one feed session. Without it
--     scores drift between calls and a keyset means nothing.
--   * the candidate pool is cut at `published_at <= anchor`, so a post
--     created while the member is paginating cannot enter a later page at
--     all. It shows up on the next refresh, at the top, where it belongs.
-- Together those make pages exactly stable across inserts: no duplicate,
-- no skipped row, and no offset anywhere.
--
-- Diversity (COMM-112) runs after scoring and reorders only the rows of the
-- page the keyset already selected, so it cannot move a row across a page
-- boundary and break that stability. The run state at the end of a page
-- travels in the cursor, so the first row of page 2 still knows what the
-- last row of page 1 was.

-- ---------------------------------------------------------------------------
-- Supporting indexes. Each one backs a lookup feed_page does per candidate
-- author or per candidate post; without them the scoring pass degrades to a
-- sequential scan per subquery.
-- ---------------------------------------------------------------------------

-- The repetition penalty and the `following` scope both walk one author's
-- recent posts.
create index if not exists workout_posts_author_recent_idx
  on public.workout_posts(author_id, published_at desc) where deleted_at is null;

-- follows is keyed (follower_id, followed_id). are_friends() and the
-- relationship score both need the reverse direction too.
create index if not exists follows_followed_idx
  on public.follows(followed_id, follower_id);

-- reactions is keyed (post_id, user_id, kind). "has the viewer reacted to
-- anything by this author lately" reads it the other way round.
create index if not exists reactions_user_idx
  on public.reactions(user_id, post_id);

-- Same shape for the comment half of the relationship score.
create index if not exists post_comments_author_recent_idx
...
declare
  ...
  -- Relationship: a mutual follow is the full component (are_friends(), the
  -- one definition of "friends" in the schema). A one-way follow is most of
  -- it. Having actually reacted to or commented on this author recently
  -- tops it up. The sum is capped at 1 before the weight applies.
    ...
    from (
      ...
      author_facts as (
        select a.aid as aid,
               least(1.0,
                 (case
                    when public.are_friends(a.aid) then v_rel_mutual
                    when exists (select 1 from public.follows f
                                 where f.follower_id = v_uid and f.followed_id = a.aid) then v_rel_follow
                    else 0
                  end)
                 + (case when exists (
                       select 1 from public.reactions r
                       join public.workout_posts rp on rp.id = r.post_id
                       where r.user_id = v_uid and rp.author_id = a.aid
                         and r.created_at >= v_anchor - make_interval(days => v_rel_window_days))
                     or exists (
                       select 1 from public.post_comments pc
                       join public.workout_posts cp on cp.id = pc.post_id
                       where pc.author_id = v_uid and cp.author_id = a.aid
                         and pc.created_at >= v_anchor - make_interval(days => v_rel_window_days))
...
-- ---------------------------------------------------------------------------
-- club_summary, COMM-115
-- ---------------------------------------------------------------------------
-- The club strip above the feed: name, mark, how many members, the one
-- active challenge, and the unread notification count. One call, because
-- five separate reads at the top of the feed is five round trips before a
-- single post renders.
create or replace function public.club_summary() returns jsonb
language plpgsql stable security definer set search_path = ''
as $$
declare
  v_uid uuid;
