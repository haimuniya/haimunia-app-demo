...
| 14 | Successful migration tests | **Passed** | Both directions. From empty: `supabase db reset` applies all 108 migrations, exit 0, PostgreSQL 17.6. **Onto existing data** (which is what `db push` actually does, and what a clean-slate test cannot cover): rolled a database back to the pre-release schema, seeded representative rows, and applied the 7 pending migrations with `supabase migration up` — clean, no FK validation failure, all row counts intact. It also proved DB-H2 was real: on the current schema a member who had created an invite **could not be deleted at all** (`23503`), which would have aborted the purge for the whole batch. See `MIGRATION_REHEARSAL.md`. |
