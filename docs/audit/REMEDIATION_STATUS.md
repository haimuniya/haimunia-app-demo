...
| VAULT-1 | Edge Function Vault secrets unset | **Open** — my earlier "Implemented" was wrong | Corrected 2026-09-08 after a peer review of the closure criterion. TWO errors, both mine. (1) The evidence was `select name from vault.decrypted_secrets;` returning both names — but 202609050005:169-183 CREATES both names itself with the placeholders `PROJECT-REF-NOT-SET` / `SERVICE-ROLE-KEY-NOT-SET`, so name-presence is true on a project where nothing was ever configured. It proved nothing. (2) I told the owner to run `vault.create_secret`, but the rows already existed on production, so those calls would have raised a duplicate-name error; the migration's own header says `vault.update_secret` for this reason. The values are therefore probably STILL placeholders. Correct fix: `vault.update_secret((select id from vault.secrets where name='…'), '…')`. Correct verification, which never prints a secret: `select name, case when decrypted_secret like '%PROJECT-REF-NOT-SET%' or decrypted_secret = 'SERVICE-ROLE-KEY-NOT-SET' then 'PLACEHOLDER' else 'set' end from vault.decrypted_secrets where name like 'edge_functions%';` Closes only when a row appears in `net._http_response` newer than the job's `start_time` — `cron.job_run_details.status` CANNOT detect this, because `cron_invoke_edge_function()` fails with `raise notice` + `return null` (202609050005:206-212), so the run records `succeeded` and `scheduled_job_health()` reports healthy in exactly the broken state. |
