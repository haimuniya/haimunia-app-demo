...
| VAULT-1 | Edge Function Vault secrets unset | **Resolved and verified** 2026-09-08 | Closed on transport evidence, not on the two signals that could not see it. Fixed with `vault.update_secret()` — my earlier `vault.create_secret()` instruction could not have worked, because 202609050005:169-183 had already created both rows on production with the placeholders. Values confirmed by a boolean check that prints no secret: both report `set`, neither still holds `PROJECT-REF-NOT-SET` / `SERVICE-ROLE-KEY-NOT-SET`. Transport then proven directly: `select public.cron_invoke_edge_function('transport-check')` returned request id 1, and `net._http_response` recorded **status_code 404 at 05:53:07 UTC, `timed_out` false, no error** — a request that left the database, reached the functions host, and was correctly told no such function exists. Deliberately probed a NON-EXISTENT slug so nothing executed. `net._http_response` is the only sound test here: `cron.job_run_details.status` and therefore `scheduled_job_health()` report `succeeded`/healthy even when the gate refuses, because `cron_invoke_edge_function()` fails with `raise notice` + `return null` (202609050005:206-212). |
