...
|---|---|---|---|
| SEC-004 | No CAPTCHA on sign-up | **Not applicable, proven with evidence** — accepted 2026-09-07 | Owner declined a third-party CAPTCHA provider. Measured what a ghost account (signed up, never redeemed an invite) can actually reach: **nothing.** `workout_posts.posts_feed_select` and `profiles_read_authenticated` are both gated on `is_community_member()`; comments, reactions and media delegate to `post_visible_to_viewer()`, which is SECURITY **INVOKER**, so the gate applies transitively through `workout_posts` RLS; every write RPC checks the same gate; the remaining un-gated policies are `_self_select`, returning the ghost's own empty rows. Invite redemption is rate-limited to 5 attempts / 15 min, so codes cannot be brute-forced either. Residual risk is empty `auth.users` rows — a quota nuisance, not data exposure. Client code stays in place and inert (`captchaSiteKey: ""`), 8 tests still green, so enabling it later is a one-line config change. |
| INF-2 | Branch protection requires all 4 CI jobs | **Resolved and verified** | Enabled by the owner 2026-09-08 and confirmed by the server itself, not by reading a settings page: a push to `main` came back with `remote: - 4 of 4 required status checks are expected.` The push was allowed because the owner is an admin and *Do not allow bypassing the above settings* is deliberately unticked — with a single committer, blocking his own pushes would buy nothing. "Require a pull request before merging" is likewise off on purpose; revisit both, together, when a second person starts committing. |
...
| VAULT-1 | Edge Function Vault secrets unset | **Resolved and verified** 2026-09-08 | `cron_invoke_edge_function('purge_abandoned_profiles')` returned **200** — `net._http_response` ids 14 and 15, 10:20:44 and 10:21:09 UTC. That is the whole chain observed, not a proxy for it: request left the database, resolved the host, passed the gateway, passed the function's OWN authorization check, and ran. Superseded three earlier closure attempts that could not see the failure — name-presence (the migration creates the names itself), `scheduled_job_health()` (the gate fails with `raise notice`, so the run records `succeeded`), and a 404 on a non-existent slug (the gateway routes before it authorises). Note for any runbook: a 401 from this path does NOT necessarily mean the edge rejected the token. `purge_abandoned_profiles/index.ts:122` timing-safe-compares the Authorization header against the service role key itself, so the gateway can accept a valid key and the function still refuse it as the wrong one. |
| DEPLOY-1 | Two of three Edge Functions were never deployed | **Resolved and verified** 2026-09-08 | Found while closing VAULT-1, and the larger finding of the two. `supabase functions list` showed only `recap_weekly` on the project. `purge_abandoned_profiles` did not exist, so the 03:31 UTC cron job had been POSTing to a 404 every night while `cron.job_run_details` recorded `succeeded`. **`admin_reset_password` was also missing, and it is not a scheduled job** — `cloud.js:5257` invokes it from an admin's browser, so a real admin resetting a member's password hit a live 404. `scheduled_job_health()` is structurally blind to this: it only knows about cron, and a client-invoked function is not in `cron.job`. Both now deployed and ACTIVE. Gated going forward by `scripts/check-edge-functions-deployed.mjs`, which needs no secret (deployed functions answer 401 to the publishable key, missing ones 404) and so is safe on forks, plus `.github/workflows/deploy-edge-functions.yml` — the absence of any automated function deploy is why two of three drifted in the first place. |
| RECAP-1 | The first weekly recap would have told inactive members they did nothing | **Resolved and verified** 2026-09-08 | Found by reading what `recap_weekly` actually SENDS before its first run at Monday 05:11 UTC, rather than only proving it could reach the network. `loadActiveMemberIds()` means "redeemed an invite and not deleted" - nothing to do with training - so every member was going to be notified, including everyone who did not train, with the body `0 אימונים השבוע, רצף של 0.`: a push telling the least engaged people in the club that they did nothing, first thing on a Monday. No test caught it and no error would ever have fired, because it was working exactly as designed - a product defect, not a fault, and the reason "the transport works" was never the same claim as "this is safe to send". Notification now guarded on the member's OWN week (sessions, PRs, achievements, personal challenge progress); the recap ROW is still written, so a member who opens the screen still sees a real zero. Club aggregates are deliberately excluded from the guard - `upcoming_event` is identical for everyone in a run, so counting it would notify the whole club whenever an event is scheduled. Fixed in `d39e8d3` and **deployed to production**, so Monday's run carries it. |
