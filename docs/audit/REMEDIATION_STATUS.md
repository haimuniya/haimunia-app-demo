...
| VAULT-1 | Edge Function Vault secrets unset | **Implemented, verification pending** — half proven, closing 03:31 UTC 2026-09-09 | Fixed with `vault.update_secret()`; my earlier `create_secret()` instruction could not have taken, since 202609050005:169-183 had already created both rows on production with placeholders. Values report `set` on a boolean check that prints no secret. **PROVEN:** base URL and transport — `cron_invoke_edge_function('transport-check')` produced `net._http_response` status_code **404 at 05:53:07 UTC**, no error, no timeout, so the URL resolves, TLS completes and the functions host answered. Probed a non-existent slug deliberately, so nothing executed. **NOT PROVEN:** the service_role key. The Functions gateway routes BEFORE it authorises, so an unknown slug returns 404 whether the key is right or wrong; a bad key surfaces as **401 on a slug that exists**, which that probe cannot produce by construction. All three real functions (`admin_reset_password`, `purge_abandoned_profiles`, `recap_weekly`) have side effects and no dry-run path, so there is no safe hand probe. Closes on tomorrow's 03:31 UTC `purge-abandoned-profiles` run, which hits a function that exists: **200 proves the key end to end; 401 means it is wrong**, with five days to fix it before `recap-weekly` on 14 Sep. That run is safe to let happen only because 202609070001 is live. Until then, "the recap sent nothing" and "the key is wrong" remain indistinguishable — the same defect as the health check. |
