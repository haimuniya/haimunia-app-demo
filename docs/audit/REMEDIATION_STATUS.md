...
| VAULT-1 | Edge Function Vault secrets unset | **Implemented, verification pending** | Both secrets created in the project Vault 2026-09-07 and confirmed present by `select name from vault.decrypted_secrets;`, which returned `edge_functions_base_url` and `edge_functions_service_role_key`. That proves the secrets EXIST, not that the jobs now do their work — `cron_invoke_edge_function()` has not been observed succeeding with them yet, and the whole point of this finding was that these jobs report `succeeded` regardless. Closes when a scheduled run is seen healthy: `purge-abandoned-profiles` daily 03:31 UTC, `recap-weekly` Mondays 05:11 UTC. Check with `select * from public.scheduled_job_health() where not healthy;`. |
